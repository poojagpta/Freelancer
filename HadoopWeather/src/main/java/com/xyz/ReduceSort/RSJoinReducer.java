...
public class RSJoinReducer extends Reducer<RSJoinCompositeKey, Text, NullWritable, Text>{
	...
	@Override
	protected void reduce(RSJoinCompositeKey key, Iterable<Text> values,
			Context context)
		...
		// Iterate through values; First set is csv of employee data
				...
				if (reduceValueBuilder.length() > 1) {
					...
					System.out.println("Key=" + key.getPlaceId() + "src="
							+ key.getNumsrc());
...
/*				Path[] cacheFilesLocal = DistributedCache.getLocalCacheArchives(context
	...
	private StringBuilder buildOutputValue(RSJoinCompositeKey key,
		...
		if (key.getNumsrc() == 1) {
			...
			reduceValueBuilder.append(key.getPlaceId()).append(strSeparator);
 
			String arrEmpAttributes[] = value.toString().split(",");
			txtMapFileLookupKey.set(arrEmpAttributes[0].toString());
			try {
